...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      // Prefer a structured vendor; fallback to rich_text; normalize for grouping
      function getVendorName(page) {
        const props = page.properties || {};
        const sel = props?.Vendor?.select?.name;
        const rel = props?.Vendor?.relation?.[0]?.name;
        const txt = props?.Vendor?.rich_text?.[0]?.plain_text;
        return (sel || rel || txt || 'Unknown').trim();
      }
      ...
      // Top vendors outstanding (normalize vendor key)
      const vendorOutstanding = {};
      payments.forEach(p=>{
        if (p.status !== 'Paid') {
          const key = (p.vendor || 'Unknown').replace(/\s+/g,' ').trim().toUpperCase();

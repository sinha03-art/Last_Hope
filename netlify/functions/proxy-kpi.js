...
body: JSON.stringify({ ok: true, msg: "kpi function up (appended handler)" })
};
};
exports.handler = async function(){
try{
if(!process.env.NOTION_API_KEY) return json(500,{error:"Missing NOTION_API_KEY"});
if(!process.env.MILESTONES_DB_ID||!process.env.CONFIG_DB_ID) return json(500,{error:"Missing DB IDs"});
const NOTION_API_KEY   = process.env.NOTION_API_KEY;
...
if ((event.queryStringParameters||{}).debug === 'ids') {
return {
statusCode: 200,
headers: { "content-type": "application/json", "Access-Control-Allow-Origin":"*" },
body: JSON.stringify({
NOTION_API_KEY: !!process.env.NOTION_API_KEY,
MILESTONES_DB_ID: process.env.MILESTONES_DB_ID || null,
CONFIG_DB_ID: process.env.CONFIG_DB_ID || null,
PAYMENT_DB_ID: process.env.PAYMENT_DB_ID || null
})
};
}
return { statusCode: 400, body: JSON.stringify({ error:"Use ?debug=ids to view IDs" }) };
};
exports.handler = async function () {
return {
statusCode: 200,

...
Issue: "${data.gateIssue}"`;
    ...
    if (event.httpMethod === 'GET') {
      ...
      const [milestonesData, deliverablesData, paymentsData, configData] = await Promise.all([
        queryNotionDB(process.env.MILESTONES_DB_ID, undefined, [{ property: 'StartDate', direction: 'ascending' }]),
        queryNotionDB(process.env.DELIVERABLES_DB_ID),
        queryNotionDB(process.env.PAYMENTS_DB_ID),
        queryNotionDB(process.env.CONFIG_DB_ID),
      ]);

...
exports.handler = async (event) => {
  ...
  try {
	if (event.httpMethod === 'POST') {
	  ...
	  if (type === 'summary') {
		const k = data.kpis || {};
		const kpiText = `Paid vs Budget: ${((k.paidVsBudget || 0) * 100).toFixed(1)}%, ` +
		  `Deliverables: ${((k.deliverablesProgress || 0) * 100).toFixed(0)}% approved, ` +
		  `Milestones At Risk: ${k.milestonesAtRisk || 0}, ` +
		...
		prompt = `Act as a renovation PM. Based on the live data, write a concise weekly update with Wins, Risks, and Next Actions.  ` +
		  `Key Metrics: ${kpiText}  Key Milestones: ${milestonesText}`;
	  } else if (type === 'suggestion') {
		prompt = `Act as a senior construction PM. A milestone is "At Risk". Provide 3 concise actions.  ` +
		  `Milestone: "${data.title}" Financial: ${data.indicator} Issue: "${data.gateIssue}"`;
	  } else {
